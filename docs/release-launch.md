...
"$SMOKE_DIR/bin/pip" install dist/ai_loop-0.2.3-py3-none-any.whl
...
git tag -a v0.2.3 "$RELEASE_SHA" -m "ailoop v0.2.3"
git push origin v0.2.3
gh release create v0.2.3 \
  dist/ai_loop-0.2.3-py3-none-any.whl \
  dist/ai_loop-0.2.3.tar.gz \
  dist/SHA256SUMS \
  --verify-tag \
  --title "ailoop v0.2.3" \
...
- `v0.2.3^{}` peels to the exact merged release commit
